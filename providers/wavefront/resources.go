	...
	Resources = []*resource.Resource{
		...
		&resource.Resource{
			...
			Arguments: []resource.Attribute{
				...
				resource.Attribute{
					Name:        "target",
					Description: `(Optional) A comma-separated list of the email address or integration endpoint (such as PagerDuty or web hook) to notify when the alert status changes. Multiple target types can be in the list. Alert target format: ({email}|pd:{pd_key}|target:{alert-target-id}).`,
				},
				resource.Attribute{
					Name:        "condition",
		...
		&resource.Resource{
			...
			Arguments: []resource.Attribute{
				...
				resource.Attribute{
					Name:        "description",
					Description: `(Required) The description of the ingestion policy ### Example ` + "`" + `` + "`" + `` + "`" + `hcl resource "wavefront_ingestion_policy" "basic" { name = "test_ingestion" description = "An ingestion policy for testing" } ` + "`" + `` + "`" + `` + "`" + ` ## Import ingestion policies can be imported using the ` + "`" + `id` + "`" + `, e.g. ` + "`" + `` + "`" + `` + "`" + ` $ terraform import wavefront_ingestion_policy.basic test_ingestion-1611946841064 ` + "`" + `` + "`" + `` + "`" + ``,
				},
			},
			Attributes: []resource.Attribute{},
		},
		&resource.Resource{
			...
			Arguments: []resource.Attribute{
				...
				resource.Attribute{
					...
					Description: `(Optional) Whether to AND source/host tags listed in relevantHostTags. If true, a source/host must contain all tags in order for the maintenance window to apply. If false, the tags are OR'ed, and a source/host must contain one of the tags. Default: false`,
				},
				resource.Attribute{
					Name:        "host_tag_group_host_names_group_anded",
					Description: `(Optional) If true, a source/host must be in 'relevantHostNames' and have tags matching the specification formed by 'relevantHostTags' and 'relevantHostTagsAnded' in order for this maintenance window to apply. If false, a source/host must either be in 'relevantHostNames' or match 'relevantHostTags' and 'relevantHostTagsAnded'. Default: false ### Example ` + "`" + `` + "`" + `` + "`" + `hcl resource "wavefront_maintenance_window" "basic" { reason = "Routine maintenance for 2020" title = "Routine maintenance" start_time_in_seconds = 1600123456 end_time_in_seconds = 1601123456 relevant_host_tags = ["my_host_tag1", "my_host_tag2"] relevant_host_tags_anded = true } ` + "`" + `` + "`" + `` + "`" + ` ## Import Maintenance windows can be imported using the ` + "`" + `id` + "`" + `, e.g. ` + "`" + `` + "`" + `` + "`" + ` $ terraform import wavefront_maintenance_window.basic 1600383357095 ` + "`" + `` + "`" + `` + "`" + ``,
				},
			},
			Attributes: []resource.Attribute{},
		},
		...
		&resource.Resource{
			...
			Arguments: []resource.Attribute{
				resource.Attribute{
					Name:        "identifier",
					Description: `(Required) The (unique) identifier of the service account to create. Must start with sa::`,
				},
				resource.Attribute{
					Name:        "active",
				...
				resource.Attribute{
					Name:        "permissions",
					Description: `(Optional) List of permission to grant to this service account. Valid options are ` + "`" + `agent_management` + "`" + `, ` + "`" + `alerts_management` + "`" + `, ` + "`" + `dashboard_management` + "`" + `, ` + "`" + `embedded_charts` + "`" + `, ` + "`" + `events_management` + "`" + `, ` + "`" + `external_links_management` + "`" + `, ` + "`" + `host_tag_management` + "`" + `, ` + "`" + `metrics_management` + "`" + `, ` + "`" + `user_management` + "`" + ``,
				},
				resource.Attribute{
					Name:        "user_groups",
