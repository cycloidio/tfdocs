	...
	Resources = []*resource.Resource{
		...
		&resource.Resource{
			...
			Arguments: []resource.Attribute{
				...
				resource.Attribute{
					...
					Description: `Array of IDs of the project SSH keys which should be added to the device. If you omit this, SSH keys of all the members of the parent project will be added to the device. If you specify this array, only the listed project SSH keys will be added. Project SSH keys can be created with the [packet_project_ssh_key](project_ssh_key.md) resource.`,
				},
				resource.Attribute{
					Name:        "type",
				...
				resource.Attribute{
					...
					Description: `String of UUID of [IP block reservations](reserved_ip_block.md) from which the public IPv4 address should be taken. You can supply one ` + "`" + `ip_address` + "`" + ` block per IP address type. If you use the ` + "`" + `ip_address` + "`" + ` you must always pass a block for ` + "`" + `private_ipv4` + "`" + `. To learn more about using the reserved IP addresses for new devices, see the examples in the [packet_reserved_ip_block](reserved_ip_block.md) documentation. ## Attributes Reference The following attributes are exported:`,
				},
				resource.Attribute{
		...
		&resource.Resource{
			...
			Arguments: []resource.Attribute{
				...
				resource.Attribute{
					Name:        "global",
					Description: `boolean flag whether addresses from a block are global (i.e. can be assigned in any facility) Idempotent reference to a first "/32" address from a reserved block might look like this: ` + "`" + `join("/", [cidrhost(packet_reserved_ip_block.myblock.cidr_notation,0), "32"])` + "`" + ``,
				},
			},
			Attributes: []resource.Attribute{
				resource.Attribute{
